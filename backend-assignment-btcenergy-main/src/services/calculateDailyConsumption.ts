...
import type { EnergyConsumptionHistory, BlockEnergyReport } from '../domain/models/energyModels';
import type {
  BlockchainBlock,
  BlocksDayResponse,
} from '../data/blockchain/models/blockchainModels';
  ...
  return async (days: number): Promise<EnergyConsumptionHistory> => {
    ...
    try {
      ...
      const dailyConsumptionPromises = dayIndices.map(async i => {
        ...
        // Limit  first 10 blocks per day for performance optimization
        ...
        const blockEnergyPromises = blocksToProcess.map(async block => {
          ...
          return calculateBlockConsumptionFn(blockData).totalEnergyConsumptionKwh;
        });
...
export const calculateDailyConsumptionPerDays = makeCreateCalculateDailyConsumptionPerDays(
