...
import type { BlockEnergyReport } from "../domain/energyModels";
...
export const makeCreateCalculateConsumptionPerBlock = (
  ...
  return async (blockHash?: string): Promise<BlockEnergyReport> => {
    try {
      ...
      throw new Error(
        `Failed to calculate consumption for block ${blockHash}: ${errorMessage}`,
      );
    }
  };
};

export const calculateConsumptionPerBlock =
  makeCreateCalculateConsumptionPerBlock(
